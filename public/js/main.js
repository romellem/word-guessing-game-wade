...
let suggest_a_category = $('#suggest-a-category');
let secret_word_toggle = $('#show-secret-word');
let secret_word_toggle_wrapper = $('.toggle-secret');

suggest_a_category.addEventListener('click', e => {
	let random_category = random_categories[randomInt(0, random_categories.length)];
	category.value = random_category;
});
